<template>
    <div data-title="A雨天-博客后台 | 评论管理">
            ...
            <el-form :inline="true" ref="queryForm" :model="queryForm" label-width="80px">
                ...
                <el-form-item>
                    ...
                    <el-button @click="check" type="error"
                        >待审核评论<span style="color: #ff180e;font-size: 15px"> {{ number }} </span><span></span
                    ></el-button>
                </el-form-item>
            </el-form>
        </div>
        ...
        <el-table :data="tableData" style="width: 100%">
            ...
            <el-table-column prop="author" label="评论者"> </el-table-column>
        ...
        <el-pagination background layout="total,prev, pager, next,jumper" @current-change="handleCurrentChange" :current-page="pageNo" :page-size="7" :total="total"> </el-pagination>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        del(row) {
            this.$confirm('确定要删除' + row.content + ' 评论吗？', '提示')
                .then(() => {
                    del(row.id)
                        .then(data => {
                            let param = this.queryForm
